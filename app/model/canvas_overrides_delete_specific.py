    ...
    if response.status_code == 200:
        ...
        for override in overrides:
            ...
            if 'student_ids' in override and student_id in override['student_ids']:
                ...
                response = requests.delete(delete_override_url, headers=headers)

                if response.status_code == 200 or response.status_code == 204:
                    print(f"Successfully deleted the override for assignment {assignment_id}")
                else:
                    print(f"Failed to delete the override for assignment {assignment_id}: {response.status_code}")
                    print(response.text)
    else:
...
def cleanup_overrides(course_id, student_ids_to_cleanup):
    ...
    print("cleanup_overrides", "course_id", course_id, "student_ids_to_cleanup", student_ids_to_cleanup)
...
if __name__ == "__main__":
    ...
    student_ids_to_cleanup = [194276]  # Add student IDs here
    course_id = '73869' 
