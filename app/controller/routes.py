...
@app.route("/overview")
def overview_new(): # Renamed function
...
@app.route("/post-pre-quiz-summary")
def post_prequiz_summary_new(): # Renamed function
    ...
    if 'quiz_response_list' in session and session['quiz_response_list'] and isinstance(session['quiz_response_list'], list) and len(session['quiz_response_list']) >= 2:
...
def report_history_new(): # Renamed function
    session_start = session.get('user_id') is not None
    curr_user_name = session.get('user_name', 'Guest')
...
@app.route("/settings")
def application_settings_new(): # Renamed function
    session_start = session.get('user_id') is not None
    curr_user_name = session.get('user_name', 'Guest')
