...
# ! to uncomment once QE2 is over. Should not need exclusion list after QE2
...
#! for QE2 with exclusion
def fetch_all_user_ids(canvas_exclusion_list=None):
    """
    Fetch all user_ids from the Students table in adq.db, excluding the student_ids corresponding to the canvas_exclusion_list.
    """
    # Fetch user_id to canvas_id mappings
    user_id_to_canvas_id, canvas_id_to_user_id = fetch_user_id_canvas_id_mapping()

    # Convert Canvas IDs to Student IDs using the mapping
    exclusion_list = []
    if canvas_exclusion_list:
        exclusion_list = [canvas_id_to_user_id.get(canvas_id) for canvas_id in canvas_exclusion_list if canvas_id_to_user_id.get(canvas_id)]
    ...
    if exclusion_list:
        ...
        sql = f"SELECT student_id FROM Students WHERE student_id NOT IN ({placeholders})"
        cursor.execute(sql, exclusion_list)
    else:
        cursor.execute("SELECT student_id FROM Students")
...
def load_ku_dict_from_db():
    """
    Load KU_dict data from the SQLite database.
    """
    ku_dict = {}
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    ...
    cursor.execute("SELECT cell, ku_values FROM KU_courseID_link")
    rows = cursor.fetchall()
...
def main():
    ...
    from canvas_overrides_app.main_parameters import due_date, course_id, time_constraint
    import requests
    import os
    import json
    from dotenv import load_dotenv
    from datetime import datetime, timedelta
    from canvas_overrides_app.UAT1b_llm_ensemble_voting_system import llm_ensemble

    # Fetch all user_ids from the database
    exclusion_list = []  #! used to filter out ctrl group ids
    user_ids = fetch_all_user_ids(exclusion_list)  # These are user_ids from adq.db
    ...
    def check_pre_quiz_completion():
        """
        Check which students have completed the pre-quiz based on the pre_quiz_completed column.
        Fetches data from the Students table and returns a dictionary where keys are user_ids,
        and values are 1 (completed) or 0 (not completed).
        """
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        cursor.execute("""
            SELECT student_id, pre_quiz_completed
            FROM Students
        """)
        rows = cursor.fetchall()
        ...
        # Create a dictionary to store the completion status
        pre_quiz_status = {user_id: completed for user_id, completed in rows}
        
        return pre_quiz_status
    
    def calculate_familiarity_index(user_ids):
        """
        Calculate the familiarity index and total score for specified user_ids based on pre-test responses.
        Fetches data from the pre-test-table and computes the familiarity score.
        Returns two dictionaries:
        - familiarity_index: where keys are user_ids and values are lists containing familiarity scores for each cell_index from 1 to 30.
        - student_scores: where keys are user_ids and values are the total scores calculated by summing learner_response values.
        """
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()

        # Initialize dictionaries to store counts and scores
        correct_counts = {user_id: {i: 0 for i in range(1, 31)} for user_id in user_ids}
        total_counts = {user_id: {i: 0 for i in range(1, 31)} for user_id in user_ids}
        student_scores = {user_id: 0 for user_id in user_ids}

        for user_id in user_ids:
            # Fetch entries from the pre-test-table for the current user_id
            cursor.execute("""
                SELECT learner_response, cell_index
                FROM pretest_table
                WHERE user_id = ?
            """, (user_id,))
            rows = cursor.fetchall()

            # Calculate correct and total counts for each cell_index, and accumulate scores
            for learner_response, cell_index in rows:
                if 1 <= cell_index <= 30:  # Ensure the cell_index is within the valid range
                    total_counts[user_id][cell_index] += 1
                    if learner_response == 1:
                        correct_counts[user_id][cell_index] += 1
                        student_scores[user_id] += 1  # Add to the student's total score
        conn.close()

        # Calculate the familiarity index for each user and prepare as lists
        familiarity_index = {}
        for user_id in user_ids:
            familiarity_index[user_id] = []
            for cell_index in range(1, 31):
                total = total_counts[user_id][cell_index]
                correct = correct_counts[user_id][cell_index]
                # Calculate familiarity score (ratio of correct responses to total responses)
                familiarity_score = correct / total if total > 0 else 0
                familiarity_index[user_id].append(familiarity_score)

        return familiarity_index, student_scores


    # Fetch quiz submission details
    print("user_ids", user_ids)
    pre_quiz_completion_status = check_pre_quiz_completion()
    print("Pre-Quiz Completion Status:", pre_quiz_completion_status)
    # fetch_quiz_submission_details()
    familiarity_index, student_scores = calculate_familiarity_index(user_ids)
    print("Familiarity Index:", familiarity_index)
    print("Student Scores:", student_scores)
    familiarity_arrays = familiarity_index
    ...
    def extract_cell_duration(KATableData):
        ...
        for KA, processes in data.items():
            for PG, details in processes.items():
                cell = details["Cell"]
                duration = details.get("Duration_min", 0)
    ...
    for user_id in user_ids:
        if pre_quiz_completion_status.get(user_id) != 1:
        ...
        student_score = student_scores.get(user_id, 0)
        ...
        pretest_scores = f"{student_score} out of 10"  # Use student's score. 10 is chosen as the total number of questions in the pre-quiz for the AQ.
        additional_custom_information = "Some cells are not being used. Ensure that only cells 1,2,3,5,18,19,20,24,25,26,27,28,29,30 are among those selected"
