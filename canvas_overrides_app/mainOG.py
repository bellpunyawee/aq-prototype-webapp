...
This script automates the creation of assignment overrides for specific students in the Canvas Learning Management System (LMS). It is designed to run indefinitely on a server, executing at regular intervals (default is every 5 minutes), 
to apply updated due dates for assignments based on predefined configurations.
...
def fetch_all_student_ids():
    """
    Fetch all student_ids from the Students table in adq.db.
    """
...
def load_student_preferences_from_db():
    ...
    Returns a dictionary with student_id as keys and their preferences and background info as values.
    """
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    ...
    for (student_id,) in students:
        ...
        if pg_prefs:
            student_preferences[student_id]["PG_preferences"] = {
                "Planning": pg_prefs[0],
        ...
        if ka_prefs:
            student_preferences[student_id]["KA_preferences"] = {
                "Resource": ka_prefs[0],
                "Risk": ka_prefs[1]
            }
        ...
        if bg_info:
            student_preferences[student_id]["background_info"] = {
                "gender": bg_info[0],
                "role": bg_info[1],
                "company": bg_info[2],
    ...
    conn.close()
    # print("Student preferences loaded from the database:", print(json.dumps(student_preferences, indent=4)))
...
def load_ku_dict_from_db():
    ...
    conn.close()
    # print("KU_dict loaded from the database:", ku_dict)
    return ku_dict

def load_ka_table_data_from_db():
    """
    This section loads KATableData data from the SQLite database and format it to match the original KATableData structure.
    """
    ...
    # Query all data from KA_table_data to populate the 'data' field
    ...
    conn.close()
    # print("KATableData loaded from the database in JSON format:")
    # print(json.dumps(ka_table_data, indent=4))
...
def main():
    ...
    # Fetch all student_ids from the database
    ...
    def extract_cell_duration(KATableData):
        ...
        for phase, processes in data.items():
    ...
    for student_id, details in student_question_details.items():
        familiarity_array = calculate_familiarity(details, quiz_cell_category)
        familiarity_arrays[student_id] = familiarity_array
    ...
    for student_id in student_ids:
        if student_id not in student_question_details:
            students_without_submissions.append(student_id)
            continue

        # Check if the student's initial_student_path is NULL in the database
        cursor.execute("SELECT initial_student_path FROM Students WHERE student_id = ?", (student_id,))
        result = cursor.fetchone()
        
        if result is not None and result[0] is not None:
            # If initial_student_path is not NULL, skip this student
            print(f"Student ID {student_id} already has an initial_student_path in the database. Skipping...")
            continue


        # Retrieve student preferences and calculated familiarity
        PG_preferences = student_preferences.get(student_id, {}).get("PG_preferences", {})
        KA_preferences = student_preferences.get(student_id, {}).get("KA_preferences", {})
        background_info = student_preferences.get(student_id, {}).get("background_info", {})
        familiarity_array = familiarity_arrays.get(student_id, [])
        
        # Retrieve student score
        student_score = student_question_details[student_id]["score"]
        print(f"Student ID: {student_id}, Score: {student_score}")
        ...
        print(f"Running LLM Ensemble for student ID {student_id}")
        ...
        student_cells[student_id] = time_limited_cells
        ...
        try:
            ...
            """, (path_str, student_id))
            conn.commit()
            print(f"Updated initial_student_path for student {student_id} with path: {path_str}")
        except sqlite3.Error as e:
            print(f"An error occurred while updating the database for student {student_id}: {e}")
    ...
    if students_without_submissions:
        ...
        for student_id in students_without_submissions:
